...
void analyse_syntaxique(Liste_hach * tab_instruction,
...
file_jeu_instruction processText(file_jeu_instruction file,
	...
	if (!strcasecmp(f->identifiant, "Etiquette")) {
	    ...
	    *co_text_attente =
		...
		if ((strcasecmp(instruction, "NOP") == 0
		    ...
		    *co_text =
			pseudo_instruction(cpt_text, instruction, co_text,
		...
		*file_erreur =
		    enfiler("Mauvaise opérande dans la section .text",
			    f->caractere, f->ligne, *file_erreur);
		f = f->suiv;
	    }
...
file_text pseudo_instruction(int *cpt_text, char *instruction,
			     file_text * co_text,
			     file_jeu_instruction * file_erreur,
	...
	*cpt_text = *cpt_text + 4;
	...
	if (strcasecmp(instruction, "LW") == 0
		...
		*cpt_text = *cpt_text + 4;
		...
		if (strcasecmp
		    (g->identifiant, "Renvoie vers une étiquette") == 0) {
...
file_jeu_instruction processBss(file_jeu_instruction file,
				file_bss * co_bss, int *cpt_bss,
				file_jeu_instruction * file_erreur,
				file_symb * co_symb,
				file_symb * co_bss_attente)
{
    file_jeu_instruction f = file;
    char copie[200];
...
file_jeu_instruction processData(file_jeu_instruction file,
		...
		if (!strcasecmp(f->caractere, ".byte")) {
			...
			*co_symb =
			...
			while (strcasecmp
			       (f->identifiant, "Retour à la ligne")) {
			    if ((!strcasecmp
				 (f->identifiant, "Valeur Décimale")
				 && (unsigned long) atoi(f->caractere) <
				 2147483647)
				||
				(!strcasecmp
				 (f->identifiant, "Valeur Hexadécimale")
				 && (unsigned long) atoi(f->caractere) <
				 4294967296)) {
				*co_data =
				    ajout_data(".word", f->ligne,
					       *cpt_data, f->caractere, 2,
					       *co_data, 0);
				*cpt_data = (*cpt_data) + 4;
			    } else {
				/*si nombre negatif */
				if (!strcasecmp
				    (f->identifiant, "Délimiteur")
				    && (!strcasecmp(f->caractere, "-"))
				    && !strcasecmp(f->suiv->identifiant,
						   "Valeur Décimale")
				    && (unsigned long) atoi(f->caractere) <
				    2147483648) {
				    char *mot2 = calloc(200, sizeof(char));
				...
				*co_symb =
				...
				while (strcasecmp
					...
					*file_erreur =
					...
					z = 0;
					*co_data =
					    ajout_data(".asciiz", f->ligne,
						       *cpt_data, mot1, 4,
						       *co_data, 0);
					for (i = 0; i < (strlen(mot1) - 1);
					     i++) {
					    if (mot1[i] == '\\'
						&& (mot1[i + 1] == 'n'
						    || mot1[i + 1] ==
...
void verif_operande(file_text co_text, file_jeu_instruction * file_erreur,
		    Liste_hach * tab_instruction,
...
void verif_registre_ope(file_jeu_instruction * file_erreur,
...
void verif_baseoffset_ope(file_jeu_instruction * file_erreur,
			  file_jeu_instruction f, file_text a)
{
...
void verif_immediat_ope(file_jeu_instruction * file_erreur,
			file_jeu_instruction f, file_text a)
{
...
void verif_shiftamount_ope(file_jeu_instruction * file_erreur,
			   file_jeu_instruction f, file_text a)
{
    if (strcasecmp(f->identifiant, "Valeur Hexadécimale") == 0
	|| strcasecmp(f->identifiant, "Valeur Décimale") == 0) {
...
void verif_relatif_ope(file_jeu_instruction * file_erreur,
		       file_jeu_instruction f, file_text a)
{
    if (strcasecmp(f->identifiant, "Valeur Hexadécimale") == 0
	|| strcasecmp(f->identifiant, "Valeur Décimale") == 0) {
...
void verif_absolu_ope(file_jeu_instruction * file_erreur,
		      file_jeu_instruction f, file_text a)
{
    if (strcasecmp(f->identifiant, "Valeur Hexadécimale") == 0
	|| strcasecmp(f->identifiant, "Valeur Décimale") == 0) {
