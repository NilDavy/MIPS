 ...
 */
int elf_get_sym_index_from_name(section symtab, section strtab,
				char *sym_name)
{
    int i = 0;
...
int elf_get_sym_index_from_name2(section symtab, section shstrtab,
 ...
 *
 */
section make_symtab_section(section shstrtab, section strtab,
 ...
 *
 */
section make_rel32_section(char *relname, table_reloc reloc,
			   section symtab, section shstrtab,
			   section strtab)
{
...
void creer_data_value(int *data_value, int *data_type, int nbdata,
...
void creer_hexa_data(file_jeu_instruction * f_data_bin, int *data_type,
...
void ordre_data(file_jeu_instruction f_data_bin, int *data_prog,
		int nbdata)
{
	...
	if (f->ligne == 2) {
		...
		if (strcmp(f->suiv->suiv->suiv->caractere, "0") == 1
		    && strcmp(f->suiv->suiv->caractere, "0") == 0
		    && strcmp(f->suiv->caractere, "0") == 0) {
		    if (strlen(f->suiv->suiv->suiv->caractere) == 1) {
			strcat(s1, "0");
		    ...
		    if (strcmp(f->suiv->suiv->suiv->caractere, "0") == 1
			&& strcmp(f->suiv->suiv->caractere, "0") == 1
			&& strcmp(f->suiv->caractere, "0") == 0) {
			if (strlen(f->suiv->suiv->suiv->caractere) == 1) {
			    strcat(s1, "0");
			}
			...
			file_symb co_symb)
{
    ...
    *data = make_data_section(data_prog, nombre_mot);
    ...
    if (!data) {
	fprintf(stderr,
		"Unable to write .data section (missing information).\n");
	exit(EXIT_FAILURE);
    }
}
...
void creer_section_text(section * text, int nbtext, file_text co_text,
...
void construction_R(int *compteur, int *text_prog, int nbre, file_text f,
...
void construction_I(int *compteur, int *text_prog, int nbre, file_text f,
		    int n, Liste_hach * tab_instruction,
		    Liste_hach * tab_registre, int *ligne,
		    file_symb co_symb)
{
			...
			if (strcasecmp
				...
				struct cellulesymb *ptr_symb =
				    recuperer_cellule_symb(g->caractere,
							   co_symb);
				convertir_dec_bin(ptr_symb->decalage, bin,
						  16);
				strcat(mot, bin);
			    } else {
				convertir_dec_bin(atoi(offset), bin, 16);
...
void construction_J(int *compteur, int *text_prog, int nbre, file_text f,
		    int n, Liste_hach * tab_instruction,
		    Liste_hach * tab_registre, int *ligne,
		    file_symb co_symb)
{
