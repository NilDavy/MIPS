					  ...
					  int noreorder)
{
	...
	return sizeof(*header32) != fwrite(header32, 1, sizeof(*header32),
					   fp);
	...
	return sizeof(*header64) != fwrite(header64, 1, sizeof(*header64),
					   fp);
				...
				unsigned int reldatasz)
{
	...
	filepos + (1 + ELF_RELOCATABLE_NSECTIONS) * sizeof(shdr[0]);
    ...
    shdr[shnum].sh_flags = 0;
    shdr[shnum].sh_link =
	elf_get_string_index(shstrtab, shstrtabsz, ".symtab");
    shdr[shnum].sh_info =
	elf_get_string_index(shstrtab, shstrtabsz, ".text");
    shdr[shnum].sh_addralign = 0;
	...
	elf_get_string_index(shstrtab, shstrtabsz, ".symtab");
    shdr[shnum].sh_info =
	elf_get_string_index(shstrtab, shstrtabsz, ".data");
    shdr[shnum].sh_addralign = 0;
    shdr[shnum].sh_entsize = sizeof(Elf32_Rel);
    END_SECTION;
    ...
    shdr[shnum].sh_link =
	elf_get_string_index(shstrtab, shstrtabsz, ".strtab");
    shdr[shnum].sh_info = 0;
    shdr[shnum].sh_addralign = 0;
	...
	!=
	fwrite(&(shdr[0]), 1,
	       (1 + ELF_RELOCATABLE_NSECTIONS) * sizeof(shdr[0]), fp)) {
	return -1;
    }
			  ...
			  unsigned char *reldata, unsigned int reldatasz)
{
	...
	for (i = 0; i < reltextsz / PELF_SIZEOF(width, Rel);
	     PELF_NEXT2(rel), i++) {
	    PELF_FLIP2(width, Rel, rel);
	}

	rel32 = (Elf32_Rel *) reldata, rel64 = (Elf64_Rel *) reldata;
	for (i = 0; i < reldatasz / PELF_SIZEOF(width, Rel);
	     PELF_NEXT2(rel), i++) {
	    PELF_FLIP2(width, Rel, rel);
	}
