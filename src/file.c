...
file_jeu_instruction enfiler(char *nom, char *carac, int line,
			     file_jeu_instruction f)
{
...
void verif_renvoie_vers_etiquette(file_jeu_instruction * f,
...
file_jeu_instruction verif_delimiteur_suite(file_jeu_instruction f,
...
file_jeu_instruction modif_chaine_caractere(file_jeu_instruction f)
{
	...
	if (strcasecmp(g->identifiant, "Chaine de caractère") == 0) {
	    ...
	    if (g->caractere[strlen(g->caractere) - 1] == '"'
		&& g->caractere[strlen(g->caractere) - 2] != '\\') {
		char modif[200] = " ";
