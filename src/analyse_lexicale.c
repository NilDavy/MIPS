...
char *analyse_lexicale(char *token, char *current_address,
...
/** Verif des caractères dans la chaine **/
		...
		if ((mot1[c] != '_') && !isalpha(mot1[c])
		    ...
		    *file_erreur =
			enfiler("Mauvais caractere dans la chaine", mot1,
				nlines, *file_erreur);
...
/** caractere numérique **/
    ...
    if (j) {
	token =
	    analyse_lexicale(mot2, current_address, delimiteur,
			     tab_registre, tab_instruction, file, nlines,
			     file_erreur, compteur);
    } else {
	if (token != NULL) {
	    token = getNextToken(current_address, delimiteur);
...
void verif_directive(char *token, char *current_address, char *delimiteur,
		     file_jeu_instruction * file, unsigned int nlines,
		     file_jeu_instruction * file_erreur, int *compteur)
{
    int i;
    int compteur1 = 0;
    char *not = "noreorder";
    char *tab[8] = { ".word", ".bss", ".data", ".text", ".byte", ".asciiz",
	".space"
    };
...
void verif_registre(char *token, Liste_hach * tab_registre,
		    file_jeu_instruction * file, unsigned int nlines,
...
void verif_caractere(char *token, char *mot2, char *current_address,
...
void verif_numero(char *token, file_jeu_instruction * file,
...
void verif_chaine_de_caractere(char *token, char *current_address,
			       char *delimiteur,
			       ...
			       int *compteur)
{
    ...
    while ((token != NULL)
	   && (token[strlen(token) - 1] != '"' || (strlen(token) == 1)
...
/** verif longueur de la ligne **/
    ...
    if (token == NULL) {
	*file_erreur =
	    enfiler("Erreur de fin de chaine", "\" de fin manquant",
		    nlines, *file_erreur);
	return;
    } else {
	*file = enfiler("Chaine de caractère", token, nlines, *file);
