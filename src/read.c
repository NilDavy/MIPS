					...
					size_t * scnsz, char *scname)
{
    ...
    *scnsz = 0;
	...
	if ((ELFCLASS64 == width
	     && !strcmp(scname, (char *) shstrtab + shdr64->sh_name))
	    || (ELFCLASS64 != width
	    ...
	    *scnsz = PELF2(width, Shdr, shdr, sh_size);
		...
		case SHT_REL:
		    for (i = 0; i < *scnsz / PELF_SIZEOF(width, Rel);
			 PELF_NEXT2(rel), i++) {
			PELF_FLIP2(width, Rel, rel);
		    }
		    break;
		case SHT_RELA:
		    for (i = 0; i < *scnsz / PELF_SIZEOF(width, Rela);
			 PELF_NEXT2(rela), i++) {
		...
		case SHT_DYNSYM:
		case SHT_SYMTAB:
		    for (i = 0; i < *scnsz / PELF_SIZEOF(width, Sym);
			 PELF_NEXT2(sym), i++) {
				    ...
				    unsigned char *shtab)
{
    ...
    scn =
	(char *) elf_read_section_by_name(fp, ehdr, shstrtab, shtab,
					  &scnsz, secname);
...
int elf_dump_section(FILE * fp, char *input, char *secname)
{
	...
	if (name) {
		...
		elf_dump_section_by_name(fp, ehdr, output, name, shstrtab,
					 shtab);
	    } else {
		if (!strcasecmp(secname, name)) {
		    elf_dump_section_by_name(fp, ehdr, output, name,
					     shstrtab, shtab);
		    break;
		}
	    }
